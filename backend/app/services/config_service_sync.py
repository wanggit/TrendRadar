...
class ConfigServiceSync:
    ...
    def get_user_config(self, user_id: int) -> UserConfig | None:
        result = self.db.execute(select(UserConfig).where(UserConfig.user_id == user_id))
        return result.scalar_one_or_none()

    def get_or_create_user_config(self, user_id: int) -> UserConfig:
    ...
    def set_ai_prompt(self, user_id: int, prompt_type: str, content: str) -> AIPrompt:
        result = self.db.execute(
            select(AIPrompt).where(AIPrompt.user_id == user_id, AIPrompt.prompt_type == prompt_type)
        )
        prompt = result.scalar_one_or_none()
    ...
    def update_user_config(self, user_id: int, update_data: ConfigUpdateRequest) -> UserConfig:
    ...
    def get_ai_prompts(self, user_id: int) -> dict:
        return {
            "interests_content": self.get_ai_prompt(user_id, "interests"),
            "classify_prompt": self.get_ai_prompt(user_id, "classify"),
            "extract_prompt": self.get_ai_prompt(user_id, "extract"),
            "update_tags_prompt": self.get_ai_prompt(user_id, "update_tags"),
            "analysis_prompt": self.get_ai_prompt(user_id, "analysis"),
        }

    def set_ai_prompts(self, user_id: int, prompts: dict) -> None:
        ...
        for field_name, content in prompts.items():
            if content is not None and field_name in field_to_type:
                ...
                self.set_ai_prompt(user_id, prompt_type, content)
    ...
    def get_system_ai_config(self) -> SystemAIConfig:
    ...
    def get_runtime_config(self, user_id: int) -> RuntimeConfig:
        config = self.get_or_create_user_config(user_id)
        user_config = self.to_full_config(config)
        ai_config = self.get_system_ai_config()
