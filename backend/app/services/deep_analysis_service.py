...
class DeepAnalysisService:
    """Deep AI analysis service for backend SaaS mode."""
    ...
    async def analyze(
        ...
        hotlist_stats, hotlist_total = await self._prepare_hotlist_stats(max_news, include_rank_timeline)
        rss_stats, rss_total = await self._prepare_rss_stats(max_news - hotlist_total) if include_rss else ([], 0)
        ...
        if include_standalone:
            standalone_content = await self._prepare_standalone(max_news)
        ...
        try:
            ...
            response = await ai_service.chat(messages, max_tokens=8000)
            result = self._parse_response(response)

            if not result.success and result.error and "JSON" in result.error:
                retry_result = await self._retry_fix_json(ai_service, response, result.error)
    ...
    async def _prepare_hotlist_stats(self, max_news: int, include_rank_timeline: bool) -> tuple[list[dict], int]:
        """
    ...
    async def _prepare_rss_stats(self, remaining: int) -> tuple[list[dict], int]:
    ...
    async def _prepare_standalone(self, max_items: int) -> str:
    ...
    async def _retry_fix_json(self, ai_service: AIService, original_response: str, error_msg: str) -> DeepAnalysisResult | None:
        """Retry JSON fix."""
        messages = [
            {
                "role": "system",
