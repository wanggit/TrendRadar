...
class StandaloneServiceSync:
    ...
    def get_standalone_items(self, standalone_config: dict) -> dict:
        """
        ...
        for source_id in selected_platforms:
            items = self._get_platform_items(source_id, limit=max_items)
            if items:
                result["platforms"][source_id] = items

        for feed_id in selected_rss_feeds:
            items = self._get_rss_items(feed_id, limit=max_items)
            if items:
                result["rss_feeds"][feed_id] = items

        return result

    def _get_platform_items(self, source_id: str, limit: int = 5) -> list[dict]:
        """Get recent news items for a specific platform source."""
        from sqlalchemy import select
        from app.models.news import Platform

        platform_result = self.db.execute(
