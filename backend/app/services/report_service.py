...
class ReportService:
    ...
    async def get_items_for_report(
        self,
        ...
        if report_mode == "current":
            news_items = await self._get_latest_news(limit)
            rss_items = await self._get_latest_rss(limit)

        elif report_mode == "daily":
            news_items = await self._get_daily_news(limit)
            rss_items = await self._get_daily_rss(limit)

        elif report_mode == "incremental":
            news_items = await self._get_incremental_news(limit, last_push_time)
            rss_items = await self._get_incremental_rss(limit, last_push_time)
    ...
    async def _get_latest_news(self, limit: int) -> list[dict]:
    ...
    async def _get_latest_rss(self, limit: int) -> list[dict]:
    ...
    async def _get_daily_news(self, limit: int) -> list[dict]:
    ...
    async def _get_daily_rss(self, limit: int) -> list[dict]:
    ...
    async def _get_incremental_news(self, limit: int, last_push_time: datetime | None) -> list[dict]:
        """Get news items since last push."""
        from sqlalchemy import select

        if not last_push_time:
            return await self._get_latest_news(limit)
    ...
    async def _get_incremental_rss(self, limit: int, last_push_time: datetime | None) -> list[dict]:
        """Get RSS items since last push."""
        from sqlalchemy import select

        if not last_push_time:
            return await self._get_latest_rss(limit)
