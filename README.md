...
> **本项目已停止开发，核心原因：无法稳定获取完整的新闻正文内容。**

### 为什么放弃？

TrendRadar 的核心价值在于**为用户提供有价值的新闻内容摘要和 AI 深度分析**。然而在实际开发和测试过程中，我们发现**获取完整新闻正文**这一基础环节存在无法逾越的障碍：

#### 1. NewsNow API 的致命缺陷

项目依赖的 [NewsNow API](https://github.com/ourongxing/newsnow) 返回的 URL 存在严重问题：

| 平台 | URL 类型 | 问题 |
|------|---------|------|
| **微博热搜** | `s.weibo.com/weibo?q=#话题#` | 返回的是**搜索页面**，不是文章链接 |
| **百度热搜** | `baidu.com/s?wd=关键词` | 返回的是**搜索页面**，不是文章链接 |
| **B站热搜** | `search.bilibili.com/all?keyword=...` | 返回的是**搜索页面**，不是文章链接 |
| **抖音热搜** | 短链接重定向 | 需要 JS 渲染，经常超时 |
| **知乎热榜** | 知乎文章链接 | 强反爬虫，100% 返回 403 |

这意味着对于**主流平台（微博、百度、B站、抖音、知乎）**，我们**根本无法获取到新闻正文**。

#### 2. 爬虫方案的局限性

即使投入大量精力开发 Scrapling 爬虫集成方案，结果仍然不理想：

| 平台 | 内容获取率 | 说明 |
|------|-----------|------|
| 36氪 | 100% | ✅ 文章链接直接可用 |
| 酷安 | 100% | ✅ 动态内容可获取 |
| GitHub Trending | 100% | ✅ 静态页面，易抓取 |
| 百度贴吧 | 69% | ⚠️ 部分可获取 |
| 少数派 | 67% | ⚠️ 部分可获取 |
| V2EX | 67% | ⚠️ 部分可获取 |
| **知乎** | **0%** | ❌ 强反爬虫，403 |
| **抖音** | **0%** | ❌ JS 渲染重，超时 |
| **今日头条** | **0%** | ❌ JS 渲染问题 |
| **Product Hunt** | **0%** | ❌ 403 反爬虫 |

**结论**：只有少数技术类平台（36氪、酷安、GitHub）能稳定获取内容，而**用户最关心的大众新闻平台（微博、知乎、抖音、头条）全部无法获取内容**。

#### 3. AI 分析失去意义

没有完整的新闻正文，AI 分析只能基于**标题**进行，这导致：

- **情感分析不准确**：标题往往带有夸张成分，无法反映真实内容
- **趋势预测不可靠**：缺少正文中的关键信息和背景
- **翻译质量差**：标题翻译无法传达完整含义
- **智能筛选失效**：基于标题的关键词匹配误判率极高

#### 4. 投入产出比极低

为了获取内容，我们尝试了：

- ✅ 静态抓取（curl_cffi + TLS 指纹伪装）
- ✅ 动态抓取（patchright + Chromium 147）
- ✅ Cloudflare 自动解决
- ✅ Bing 搜索回退（搜索标题找文章）
- ✅ 多策略内容提取（文章选择器 + 段落提取 + body 回退）

即使投入了如此多的技术方案，**核心问题依然无解**：微博、百度等平台返回的就是搜索链接，这不是技术问题，是**数据源本身的设计问题**。
...
本项目作为**标题聚合 + AI 标题分析**工具仍然可用，但请明确：
...
> 💡 新增 SaaS 多用户模式，支持邮箱注册登录、多租户数据隔离、可视化配置管理
...
| 角色 | 邮箱 | 密码 |
|------|------|------|
| 管理员 | admin@trendradar.com | admin123456 |
| 测试用户 | demo@test.com | demo123456 |

### 首次使用

1. 运行 `./start.sh start` 自动安装依赖并启动
2. 打开浏览器访问 http://localhost:5173
3. 使用默认账号登录，或注册新账号
4. 在「配置管理」页面设置你关注的热榜平台、关键词、推送渠道
5. 在「任务调度」页面手动触发爬虫或查看定时任务状态
...
> TrendRadar 已从命令行工具升级为完整的 SaaS 多用户平台，提供可视化配置管理、多租户数据隔离、用户权限控制等企业级功能。
...
告别手动编辑 YAML 文件！TrendRadar 提供完整的可视化配置管理界面，所有配置项均可通过 Web 界面一键设置。

#### 配置模块一览

| 模块 | 功能 | 核心能力 |
|------|------|---------|
| **📋 热榜平台** | 平台管理 | 添加/删除/启用/禁用平台，拖拽调整优先级，成功率标识 |
| **📡 RSS 订阅** | RSS 源管理 | 添加 RSS 源，设置更新频率，关键词过滤 |
| **📄 报告模式** | 输出格式 | 选择报告样式，自定义展示风格 |
| **🔍 筛选策略** | 新闻过滤 | 关键词匹配、正则表达式、全局过滤 |
| **🤖 AI 智能筛选** | AI 打分 | 自然语言描述关注方向，AI 自动提取标签并打分 |
| **📺 推送内容控制** | 显示控制 | 区域显示开关、排序自定义、每组数量限制 |
| **🔔 推送通知** | 渠道配置 | 飞书、钉钉、企业微信、Telegram、邮件、Slack 等 9 个渠道 |
| **⏰ 调度设置** | 定时任务 | 推送时间窗口、频率控制、去重策略 |
| **📅 调度时间线** | 可视化排程 | 7×24 周视图，拖拽设置时间段，预设模板选择 |
| **🔑 关键词** | 关键词管理 | 分组管理、正则语法、显示名称、数量限制 |
| **🧠 AI 分析** | 深度分析 | 模型配置、提示词设置、分析范围控制 |
| **🌐 AI 翻译** | 多语言翻译 | 翻译开关、目标语言、翻译风格自定义 |
| **⚙️ 高级设置** | 系统配置 | 存储配置、时区设置、调试模式 |

#### 配置管理功能

- **导出配置**: 一键导出 JSON 配置文件，方便备份和迁移
- **导入配置**: 支持从 JSON 文件导入配置，快速恢复设置
- **版本对比**: 对比当前配置与默认值，清晰展示已修改的项
- **实时保存**: 修改后实时同步到后端，无需手动保存文件

### 👥 多租户架构

| 特性 | 说明 |
|------|------|
| **用户注册/登录** | 邮箱注册，支持邮箱验证、密码重置 |
| **数据隔离** | 每个用户的数据完全隔离，平台、RSS、新闻、配置各自独立 |
| **权限控制** | 管理员、普通用户角色区分，管理员可管理所有用户 |
| **订阅体系** | 免费版/专业版/企业版，不同等级解锁不同功能 |

### 📊 功能模块

| 模块 | 说明 |
|------|------|
| **Dashboard** | 数据概览，任务统计，平台状态 |
| **新闻浏览** | 查看抓取的历史新闻，支持筛选和搜索 |
| **任务管理** | 手动触发爬虫任务，查看任务历史和日志 |
| **配置中心** | 13 个配置模块，可视化编辑所有设置 |
| **账号管理** | 个人信息修改，密码修改，订阅管理 |
| **付费管理** | 套餐购买，订单管理，升级专业版 |

### 🔐 安全特性

- **JWT 认证**: 前后端分离架构，使用 JWT Token 进行身份验证
- **密码加密**: 使用 bcrypt 加密存储用户密码
- **邮箱验证**: 注册时需要验证邮箱，防止恶意注册
- **CSRF 防护**: 关键操作需要额外验证
- **速率限制**: API 接口防刷保护
...
> TrendRadar SaaS 平台集成了完整的支付与订阅功能，支持支付宝/微信支付，提供免费版/专业版/企业版三级订阅体系。

### 订阅体系

| 等级 | 价格 | 平台数 | 关键词组 | 每日推送 | 通知渠道 | AI 功能 | 数据保留 |
|------|------|--------|---------|---------|---------|---------|---------|
| **免费版** | 免费 | 3 个 | 5 组 | 4 次 | 1 个 | ❌ | 30 天 |
| **专业版** | ¥49/月 | 15 个 | 无限 | 48 次 | 3 个 | ✅ | 30 天 |
| **企业版** | 定制 | 无限 | 无限 | 无限 | 无限 | ✅ | 365 天 |

### 付费套餐

| 套餐 | 时长 | 价格 | 日均成本 | 优惠 |
|------|------|------|---------|------|
| **月卡** | 30 天 | ¥49.00 | ¥1.63/天 | - |
| **季卡** | 90 天 | ¥129.00 | ¥1.43/天 | 省 ¥18 |
| **年卡** | 365 天 | ¥399.00 | ¥1.09/天 | 省 ¥189 |
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  前端选择套餐 ──▶ 创建订单 ──▶ Z-Pay 支付网关 ──▶ 支付宝/微信支付    │
│       │              │              │                    │          │
│       │              │              │                    │          │
│       │              ▼              ▼                    ▼          │
│       │         订单记录      MD5 签名验证          用户扫码支付     │
│       │              │              │                    │          │
│       │              │              │                    │          │
│       │              ▼              ▼                    │          │
│       │         30分钟超时     异步回调通知 ◀───────────┘          │
...
TrendRadar 使用 **Z-Pay** 作为统一支付网关，支持支付宝和微信支付。

#### 环境变量配置

| 变量 | 说明 | 示例 |
|------|------|------|
| `ZPAY_UID` | Z-Pay 商户 ID | `1001` |
| `ZPAY_KEY` | Z-Pay 商户密钥 | `your-secret-key` |
| `ZPAY_API_URL` | Z-Pay API 地址 | `https://zpayz.cn` |

#### 配置文件位置

```
backend/app/core/config.py     # 配置定义
backend/app/core/constants.py  # 产品价格定义
backend/app/api/payment.py     # 支付 API 路由
backend/app/services/payment/  # 支付服务实现
    ├── __init__.py
    └── zpay_service.py        # Z-Pay 服务
```
...
| 接口 | 方法 | 说明 | 认证 |
|------|------|------|------|
| `/api/v1/payment/create` | POST | 创建订单，获取支付链接 | JWT |
| `/api/v1/payment/callback/zpay` | POST | Z-Pay 异步回调通知 | 签名验证 |
| `/api/v1/payment/orders` | GET | 获取用户订单列表 | JWT |
| `/api/v1/payment/orders/{id}` | GET | 获取订单详情 | JWT |
| `/api/v1/payment/orders/{id}/status` | GET | 查询订单状态（主动轮询） | JWT |

### 核心功能

#### 1. 订单创建

- 自动生成订单号（格式：`TR20260513120000a1b2c3d4`）
- 订单 30 分钟超时
- 支持支付宝/微信支付选择
- 调用 Z-Pay API 获取支付链接
...
| `trade_no` | VARCHAR(128) | 支付平台交易号 |
| `product_type` | ENUM | 套餐类型（monthly/quarterly/yearly） |
| `amount` | DECIMAL(10,2) | 支付金额 |
| `payment_method` | ENUM | 支付方式（alipay/wxpay） |
| `status` | ENUM | 订单状态（pending/paid/failed/expired） |
| `paid_at` | DATETIME | 支付时间 |
| `expire_at` | DATETIME | 订单超时时间 |
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                            TrendRadar v7.0 技术架构                              │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌───────────────────────────────── 展示层 ─────────────────────────────────┐   │
│  │                                                                          │   │
│  │  Vue 3 + Vite 6 + Element Plus 2.9 + Pinia + Vue Router                 │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐  │   │
│  │  │ Dashboard│ │ 新闻浏览  │ │ 配置中心  │ │ 任务管理  │ │ 账号/付费管理 │  │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘  │   │
│  │                                                                          │   │
│  └──────────────────────────────┬───────────────────────────────────────────┘   │
│                                 │ HTTP/REST + JWT                                │
│  ┌───────────────────────────────── 应用层 ─────────────────────────────────┐   │
│  │                                                                          │   │
│  │  FastAPI 0.115 + Uvicorn + Pydantic 2.10                                │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐  │   │
│  │  │ auth API │ │ config   │ │ data API │ │ task API │ │ payment API  │  │   │
│  │  │          │ │ API      │ │          │ │          │ │              │  │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘  │   │
│  │                                                                          │   │
│  └──────────────────────────────┬───────────────────────────────────────────┘   │
...
│  │                                                                          │   │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐   │   │
│  │  │ AI 服务      │ │ 配置服务      │ │ 数据服务      │ │ 报告服务      │   │   │
│  │  │ LiteLLM      │ │ YAML/DB同步   │ │ 新闻/RSS     │ │ HTML/推送    │   │   │
│  │  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘   │   │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐   │   │
│  │  │ 爬虫服务      │ │ 推送服务      │ │ 邮件服务      │ │ 审计服务      │   │   │
│  │  │ Scrapling    │ │ 9渠道推送     │ │ SMTP发送     │ │ 操作日志     │   │   │
│  │  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘   │   │
│  │                                                                          │   │
│  └──────────────────────────────┬───────────────────────────────────────────┘   │
│                                 │ 任务分发                                       │
...
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐  │   │
│  │  │ crawl    │ │ analyze  │ │ push     │ │translate │ │  data_cleanup│  │   │
│  │  │ 爬虫任务  │ │ AI分析   │ │ 推送任务  │ │ AI翻译   │ │   数据清理    │  │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘  │   │
│  │                                                                          │   │
│  └──────────────────────────────┬───────────────────────────────────────────┘   │
│                                 │ 数据持久化                                     │
...
│  │                                                                          │   │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐   │   │
│  │  │  MySQL       │ │   Redis      │ │  本地文件     │ │  远程存储     │   │   │
│  │  │  SQLAlchemy  │ │   缓存/队列   │ │  YAML配置    │ │  S3/R2       │   │   │
│  │  │  Alembic迁移 │ │              │ │  HTML报告    │ │              │   │   │
│  │  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘   │   │
│  │                                                                          │   │
│  └──────────────────────────────────────────────────────────────────────────┘   │
...
| 技术 | 版本 | 用途 | 说明 |
|------|------|------|------|
| **Vue 3** | 3.5.13 | 核心框架 | Composition API，响应式系统 |
| **Vite** | 6.0.5 | 构建工具 | 快速热更新，按需编译 |
| **Element Plus** | 2.9.1 | UI 组件库 | 企业级组件，主题定制 |
| **Pinia** | 2.3.0 | 状态管理 | 轻量级，TypeScript 友好 |
| **Vue Router** | 4.5.0 | 路由管理 | 路由守卫，懒加载 |
| **Axios** | 1.7.9 | HTTP 客户端 | 请求拦截，Token 自动附加 |
| **Day.js** | 1.11.13 | 时间处理 | 轻量级日期库 |
| **Playwright** | latest | E2E 测试 | 浏览器自动化测试 |

#### 后端技术栈

| 技术 | 版本 | 用途 | 说明 |
|------|------|------|------|
| **FastAPI** | 0.115.6 | Web 框架 | 异步处理，自动 API 文档 |
| **Uvicorn** | 0.34.0 | ASGI 服务器 | 高性能异步服务器 |
| **SQLAlchemy** | 2.0.36 | ORM 框架 | 数据库抽象，异步支持 |
| **Alembic** | 1.14.0 | 数据库迁移 | 版本控制，自动迁移 |
| **Pydantic** | 2.10.3 | 数据验证 | 请求/响应模型验证 |
| **Celery** | 5.4.0 | 任务队列 | 异步任务，定时调度 |
| **Redis** | 5.2.1 | 缓存/消息代理 | 任务队列，会话存储 |
| **python-jose** | 3.3.0 | JWT 认证 | Token 生成/验证 |
| **passlib+bcrypt** | 1.7.4 | 密码加密 | 安全密码存储 |

#### 爬虫与 AI 技术栈

| 技术 | 版本 | 用途 | 说明 |
|------|------|------|------|
| **Scrapling** | 0.4.8 | 内容提取 | 自适应解析器 |
| **curl_cffi** | 0.15.0 | 静态抓取 | TLS 指纹伪装 |
| **patchright** | 1.59.1 | 动态抓取 | Playwright 防检测分支 |
| **LiteLLM** | 1.82.6 | AI 统一接口 | 支持 100+ AI 提供商 |
| **feedparser** | 6.0.12 | RSS 解析 | RSS/Atom 解析 |
| **requests** | 2.33.0 | HTTP 客户端 | 同步请求 |
| **httpx** | 0.28.1 | HTTP 客户端 | 异步请求 |

#### 存储与部署

| 技术 | 用途 | 说明 |
|------|------|------|
| **MySQL** | 主数据库 | 用户数据、配置、新闻存储 |
| **Redis** | 缓存/队列 | Celery 任务队列，会话缓存 |
| **S3/R2** | 远程存储 | 配置文件、报告备份 |
| **Docker** | 容器化部署 | 多架构支持，一键部署 |
| **GitHub Actions** | CI/CD | 自动化测试，定时爬虫 |
...
```
用户注册 → 邮箱验证 → JWT Token → API 访问
         ↓
    密码重置 ← 忘记密码
```

- **JWT 认证**: 访问 Token + 刷新 Token 双机制
- **权限控制**: 管理员/普通用户角色区分
- **订阅管理**: 免费版/专业版/企业版功能限制
- **审计日志**: 记录关键操作，支持追溯

#### 2. 配置管理模块 (`config`)

```
前端表单 → API 接收 → Pydantic 验证 → DB 存储
                                    ↓
                              YAML 文件同步 ← 配置文件版本管理
                                    ↓
                              版本对比 ← 导出/导入
...
```
NewsNow API → URL 分类 → 抓取策略路由 → 内容提取 → DB 存储
                    ↓
              静态抓取 (curl_cffi)
              动态抓取 (patchright)
              RSS 解析 (feedparser)
...
- **多源采集**: NewsNow API + RSS 订阅
- **智能路由**: 根据平台特性选择抓取策略
- **内容提取**: Scrapling 三级回退策略
- **去重机制**: URL 标准化，标题去重

#### 4. AI 分析模块 (`analyze`)

```
新闻数据 → LiteLLM → AI 分析 → 结果存储 → 推送展示
              ↓
         智能筛选 / 深度分析 / 多语言翻译
```

- **统一接口**: LiteLLM 支持 100+ AI 提供商
- **智能筛选**: 自然语言描述，AI 自动提取标签
- **深度分析**: 趋势概述、情感分析、异动检测
- **多语言翻译**: 批量翻译，自定义翻译风格
- **智能省钱**: 已分析新闻不重复消耗 token

#### 5. 任务调度模块 (`tasks`)

```
Celery Beat → 定时触发 → 任务队列 → Worker 执行 → 结果记录
              ↓
         crawl → analyze → translate → push → cleanup
```

- **定时调度**: Celery Beat 定时任务
- **异步执行**: Celery Worker 异步处理
- **任务日志**: 实时进度，详细日志记录
- **错误重试**: 自动重试，失败告警
...
分析结果 → 渠道适配 → 分批发送 → 推送成功
              ↓
    飞书 / 钉钉 / 企业微信 / Telegram / 邮件 / Slack / ntfy / Bark / Webhook
```
...
| 模型 | 说明 | 关键字段 |
|------|------|---------|
| **User** | 用户信息 | id, email, password_hash, tier, is_active |
| **UserConfig** | 用户配置 | id, user_id, config_key, config_value |
| **Platform** | 热榜平台 | id, user_id, source_id, name, enabled |
| **NewsItem** | 新闻条目 | id, user_id, platform_id, title, url, rank, content |
| **RSSFeed** | RSS 源 | id, user_id, feed_url, name, feed_key |
| **RSSItem** | RSS 条目 | id, user_id, feed_id, title, url, summary |
| **TaskLog** | 任务日志 | id, user_id, task_id, status, progress, result |
| **Order** | 订单 | id, user_id, amount, status, tier |
| **AuditLog** | 审计日志 | id, user_id, action, details, created_at |
...
本项目使用 Playwright 进行前端端到端（E2E）测试，覆盖登录、注册、配置页等核心功能。
...
# 运行特定测试文件
npx playwright test tests/e2e/config.spec.js

# 运行特定测试用例（按名称过滤）
npx playwright test -g "报告模式"

# 以有头模式运行（可见浏览器窗口，便于调试）
...
```bash
# 打开 Playwright UI 模式，可逐条查看测试执行过程
...
| 测试文件 | 覆盖功能 |
|---------|---------|
| `login.spec.js` | 登录页展示、表单验证、跳转注册 |
| `register.spec.js` | 注册页展示、跳转登录 |
| `config.spec.js` | 配置页全部 13 个模块：热榜平台、RSS 订阅、报告模式、筛选策略、AI 智能筛选、推送内容控制、推送通知、调度设置、调度时间线、关键词、AI 分析、AI 翻译、高级设置，以及导出/导入/版本对比功能 |
...
|   |   |   |
|:---:|:---:|:---:|
| [🚀 **一键启动**](#-一键启动saas-模式) | [🚀 **快速开始**](#-快速开始) | [AI 智能分析](#-ai-智能分析) |
| [⚙️ **配置详解**](#配置详解) | [Docker部署](#6-docker-部署) | [MCP客户端](#-mcp-客户端) |
| [📝 **更新日志**](#-更新日志) | [🎯 **核心功能**](#-核心功能) | [☕ **支持项目**](#-支持项目) |
...
> **状态**: 内容获取功能已验证可行，75% 直接链接平台可成功获取内容

### 📊 项目进度

| 模块 | 状态 | 说明 |
|------|------|------|
| Scrapling 框架集成 | ✅ 完成 | 静态抓取 + 动态抓取双引擎 |
| 内容提取策略 | ✅ 完成 | 多选择器 + 段落提取 + body 回退 |
| 直接链接平台测试 | ✅ 完成 | 6/8 平台成功获取内容 |
| 搜索链接平台处理 | ✅ 完成 | 跳过内容获取，仅抓取标题 |
| AI 分析使用内容 | ✅ 完成 | 内容已用于 AI 深度分析 |
| 端到端流程验证 | ✅ 完成 | 抓取 → 内容 → AI 分析全流程 |
...
| 组件 | 库 | 版本 | 用途 | 优势 |
|------|-----|------|------|------|
| **静态抓取** | `curl_cffi` | 0.15.0 | TLS 指纹伪装，模拟 Chrome 浏览器 | 速度快，资源占用低，适合静态页面 |
| **动态抓取** | `patchright` | 1.59.1 | Playwright 防检测分支，Chromium 147 | 处理 JS 渲染，隐藏 Headless 特征 |
| **内容提取** | `Scrapling` | 0.4.8 | 自适应解析器，智能内容提取 | 多策略回退，抗网站结构变化 |
| **搜索回退** | `requests` + `BeautifulSoup` | - | Bing 搜索获取真实文章链接 | 处理搜索链接平台 |

#### 反爬虫策略

| 策略 | 实现方式 | 适用场景 |
|------|---------|---------|
| **TLS 指纹伪装** | 自动模拟最新 Chrome 浏览器 TLS 指纹 | 静态抓取，绕过 TLS 指纹检测 |
| **浏览器自动化** | patchright 驱动 Chromium，隐藏 Headless 特征 | 动态抓取，处理强反爬网站 |
| **请求头伪装** | 自动生成真实浏览器请求头（stealthy_headers） | 所有抓取场景 |
| **Cloudflare 处理** | 动态抓取支持自动解决 Cloudflare 验证 | 有 Cloudflare 保护的网站 |
| **Canvas 指纹噪声** | 添加随机噪声防止指纹追踪 | 动态抓取，防止浏览器指纹识别 |
| **WebRTC 防护** | 强制 WebRTC 尊重代理设置，防止 IP 泄漏 | 动态抓取，保护真实 IP |
| **资源禁用** | 禁用图片、字体等非必要资源 | 动态抓取，提升速度 30%+ |
| **广告拦截** | 自动拦截广告和追踪脚本 | 动态抓取，减少干扰 |

#### 内容提取策略

采用 **三级回退策略**，确保最大化内容获取率：
...
第一级：文章选择器匹配
  ...
  ├─ .detail-content, .newsflash-item (36kr)
  ├─ .topic_content (V2EX)
  └─ .main-content, .article-body, .story-content
       │
       ▼ (失败或内容<50字)
第二级：段落提取
  ├─ 获取所有 <p> 标签
...
| 平台类型 | 抓取策略 | 超时设置 | 特殊处理 |
|---------|---------|---------|---------|
| **静态新闻站** (36氪、少数派) | 静态优先，动态回退 | 10s 静态 / 15s 动态 | 文章选择器匹配 |
| **论坛社区** (V2EX、贴吧) | 动态优先 | 15s 动态 | 段落提取为主 |
| **JS 重渲染** (酷安、GitHub) | 动态抓取 | 15s 动态 | 禁用资源加速 |
| **强反爬** (知乎) | 跳过内容获取 | - | 返回 403，记录日志 |
| **搜索链接** (微博、百度) | 仅标题 | - | Bing 搜索回退 (可选) |

#### 性能优化

| 优化项 | 措施 | 效果 |
|--------|------|------|
| **平台优先级** | 高成功率平台优先抓取 | 减少无效等待时间 |
| **低成功率跳过** | 自动跳过 0% 成功率平台 | 节省 30%+ 任务时间 |
| **搜索链接跳过** | 跳过搜索链接平台内容获取 | 避免 Bing 搜索耗时 |
| **资源禁用** | 动态抓取禁用图片/字体 | 提升 30% 抓取速度 |
| **速率限制** | 请求间隔 0.2s | 避免触发反爬 |
| **内容长度限制** | 最大 10000 字符 | 减少存储和传输开销 |
...
if result.success:
    ...
    # 反爬：记录日志，跳过
    log.warning(f"Anti-scraping: {url}")
elif result.error == "timeout":
    # 超时：可重试一次
    retry_fetch(url, timeout=20000)
else:
    # 其他错误：记录并跳过
    log.debug(f"Fetch failed: {result.error}")
```
...
| 平台 | 内容获取率 | 问题 | 建议 |
|------|-----------|------|------|
| **Product Hunt** | 0% | 403 反爬虫 | 暂时禁用 |
| **今日头条** | 0% | JS 渲染问题 | 需要动态抓取优化 |
| **知乎热榜** | 0% | 强反爬虫 403 | 跳过内容获取 |
| **抖音热搜** | 0% | 超时/JS 重 | 跳过内容获取 |

#### ❌ 搜索链接平台（无法直接获取内容）

| 平台 | URL 类型 | 说明 |
|------|---------|------|
| **微博热搜** | `s.weibo.com/weibo?q=#话题#` | NewsNow 返回搜索页，非文章链接 |
| **百度热搜** | `baidu.com/s?wd=关键词` | NewsNow 返回搜索页，非文章链接 |
| **B站热搜** | `search.bilibili.com/all?keyword=...` | NewsNow 返回搜索页，非文章链接 |

### 🎯 当前问题

1. **NewsNow API 限制**: 对于微博、百度等平台，返回的是搜索链接而非文章链接，这是 NewsNow API 的设计问题
2. **知乎反爬虫**: 知乎有极强的反爬虫机制，即使使用 DynamicFetcher 也返回 403
3. **抖音超时**: 抖音页面 JS 渲染重，DynamicFetcher 经常超时（30秒）
4. **任务执行顺序**: 搜索链接平台的内容获取（Bing 搜索回退）非常耗时，导致任务卡住

### 💡 建议策略

1. **保留高成功率平台**: 36氪、酷安、GitHub、少数派、V2EX、贴吧
2. **移除低成功率平台**: Product Hunt、今日头条
3. **搜索链接平台**: 保留标题抓取，跳过内容获取
4. **知乎/抖音**: 需要特殊处理或暂时禁用
5. **优先使用 RSS 源**: RSS 天生自带内容，100% 覆盖率
...
const PRESET_PLATFORMS = [
  // 高成功率平台（推荐）
  { source_id: '36kr', name: '36氪' },
  { source_id: 'coolapk', name: '酷安' },
  { source_id: 'github', name: 'GitHub Trending' },
  { source_id: 'tieba', name: '百度贴吧' },
  { source_id: 'sspai', name: '少数派' },
  { source_id: 'v2ex', name: 'V2EX' },
  { source_id: 'ithome', name: 'IT之家' },
  { source_id: 'huxiu', name: '虎嗅' },
  
  // 低成功率平台（不推荐）
  { source_id: 'zhihu', name: '知乎热榜' },
  { source_id: 'douyin', name: '抖音热搜' },
  { source_id: 'toutiao', name: '今日头条' },
  { source_id: 'producthunt', name: 'Product Hunt' },
  
  // 搜索链接平台（仅标题）
  { source_id: 'weibo', name: '微博热搜' },
  { source_id: 'baidu', name: '百度热搜' },
  { source_id: 'bilibili', name: 'B站热搜' },
]
```

### 🔄 下一步计划

- [x] 优化任务执行顺序，优先抓取直接链接平台
- [x] 禁用搜索链接平台的内容获取（节省时间）
- [x] 前端添加平台成功率标识
- [ ] 为知乎/抖音寻找替代方案（如第三方 API）
- [ ] 增加更多高成功率平台（如 IT之家、虎嗅等）
- [ ] 优化 DynamicFetcher 超时设置
